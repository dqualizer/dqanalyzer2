...
export default function replacePlaceholders(
  ...
  let result = sentencePart.description.replace(
    placeholderRegex,
    (match, placeholder) => {
      if (
        placeholder === "are" ||
        ...
        const allPlaceholders = sentencePart.description
          .match(placeholderRegex)
          .map((match) => match.replace(/^\[|\]$/g, ""));
        const noun = allPlaceholders.find((element) => element !== placeholder);
        let allFittingElement;
        if (noun === "actor") {
          allFittingElement = words.filter(
            (word) => word.type === "person" || word.type === "system",
          );
  ...
  result = result.replace(placeholderRegex, (match, placeholder) => {
    if (words.some((item) => fitsIn(item.type, placeholder))) {
      if (part === "speakers") {
        ...
        if (
          areValidElementsForDescription(
            examiningElements,
            sentencePart.number_actor,
          )
        ) {
          ...
          for (const examiningElement of examiningElements) {
            ...
            if (examiningElement.is_proper_noun === true) {
              ...
              sentencePart.number_actor === "plural" &&
        ...
        if (placeholder === "verb") {
          ...
          if (
            areValidElementsForDescription(
              examiningElements,
              sentencePart.number_work_object,
            )
          ) {
            const replacingStringList = [];
            for (const examiningElement of examiningElements) {
              ...
              if (examiningElement.is_proper_noun === true) {
                ...
                examiningElement.is_proper_noun === false &&
                examiningElement.number === "singular" &&
                sentencePart.number_work_object === "singular"
              ) {
                elementName = indefinite(elementName);
              }
              if (
                sentencePart.number_work_object === "plural" &&
                examiningElement.number === "singular"
              ) {
