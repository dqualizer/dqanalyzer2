...
export default function ScenarioDescriptionFormatter(scenario) {
  const formatRqsDescription = () => {
    ...
    if (optionsToFormat !== null) {
      resultString = resultString.replace(
        optionsToFormat,
        `<span class="bold-text">${optionsToFormat}</span>`,
      );
    }
    if (expectedToFormat !== null) {
      resultString = resultString.replace(
        expectedToFormat,
        `<span class="bold-text">${expectedToFormat}</span>`,
      );
    }
    if (loadToFormat !== null) {
      resultString = resultString.replace(
        loadToFormat,
        `<span class="bold-text">${loadToFormat}</span>`,
      );
    }
    if (resilienceToFormat !== null) {
      resultString = resultString.replace(
        resilienceToFormat,
        `<span class="bold-text">${resilienceToFormat}</span>`,
      );
    }
  ...
  const formatPart = (part) => {
    if (part === "Options") {
      if (
        ...
        scenario.attachment.includes("[options]")
      ) {
      ...
      if (
        scenario.attachment !== null &&
        scenario.attachment.includes("[expected]")
      ) {
  ...
  return (
    <p
      ...
      dangerouslySetInnerHTML={{ __html: formattedRqsDescription }}
    ></p>
  );
}
