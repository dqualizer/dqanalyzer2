...
class ScenarioService {
  colorActiveActivities(event, reactFlowInstance) {
    const relatedEdgesArray = reactFlowInstance
      .getEdges()
      .filter((edge) => edge.name === event.target.value);
    const unrelatedEdgesArray = reactFlowInstance
      .getEdges()
      .filter((edge) => edge.name !== event.target.value);

    for (const edge of unrelatedEdgesArray) {
      edge.selected = false;
    }

    for (const edge of relatedEdgesArray) {
      edge.selected = true;
    }
  ...
  getRqsForActivityAndMode = (activity, mode, nodes, edges) => {
    const wordArray = ActivityParser(nodes, edges, activity);

    const generatedRqs = RqsGeneratorService(mode, wordArray);
  ...
  isActivityViewInvalid(isDisabled, allDefinedScenarios) {
    for (const scenario of allDefinedScenarios) {
      if (
        ...
        scenario.selected_mode === "What if" &&
        (scenario.load_design?.design_parameters.find(
          (param) => param.value === null,
        ) ||
          scenario.resilience_design?.design_parameters.find(
            (param) => param.value === null,
          ))
      ) {
        isDisabled = true;
  ...
  isVariantAvailable(scenario, variant, isLoad) {
    if (isLoad) {
      return scenario.metric_load.load_variants.some(
        (loadVariant) => loadVariant === variant.name.toLowerCase(),
      );
    }
    return scenario.metric_resilience.resilience_variants.some(
      (resilienceVariant) => resilienceVariant === variant.name.toLowerCase(),
    );
  }
}
