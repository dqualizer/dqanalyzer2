...
export default function PlaceholderWordMapperService(sentence, part) {
  ...
  const areValidElementsForDescription = (examiningElements, condition) => {
    for (const examiningElement of examiningElements) {
      if (
        examiningElement.type === "person" ||
        examiningElement.type === "system" ||
        examiningElement.type === "work object"
      ) {
        if (
          condition !== null &&
          condition !== examiningElement.number &&
          examiningElement.is_proper_noun
        ) {
          return false;
        }
      }
  ...
  if (part === "Mandatory") {
    ...
    for (const match of matches) {
      if (
        sentence.words.speakers.some((speaker) =>
          fitsIn(speaker.type, match[1]),
        )
      ) {
        const isPossibleDescription = areValidElementsForDescription(
          sentence.words.speakers,
          mandatoryDetails.number_actor,
        );
        ...
        for (const speaker of sentence.words.speakers) {
          ...
          if (speaker.is_proper_noun === true) {
            ...
            mandatoryDetails.number_actor === "plural" &&
            speaker.number === "singular" &&
            speaker.is_proper_noun === false
          ) {
            ...
            (speaker.type === "person" || speaker.type === "system") &&
            speaker.is_proper_noun === false &&
            speaker.number === "singular"
          ) {
            elementName = indefinite(elementName);
        ...
        sentence.words.message.some((message) => fitsIn(message.type, match[1]))
      ) {
        if (match[1] === "verb") {
          ...
          if (mandatoryDetails.form_verb === "infinitive") {
            ...
            verbForm = compromise(verb)
              .verbs()
              .toGerund()
              .out("text")
              .replace(/^(is |am |are |was |were )?/, "");
          }
          ...
          for (const examiningElement of examiningElements) {
            ...
            if (examiningElement.is_proper_noun === true) {
              ...
              mandatoryDetails.number_work_object === "singular"
            ) {
              elementName = indefinite(elementName);
            }
            if (
              mandatoryDetails.number_work_object === "plural" &&
              examiningElement.number === "singular"
            ) {
        ...
        mandatoryWordMap[match[0]] =
          VerbConjugationService.getConjugatedVersionOfVerb(
            sentence.words.speakers,
            match[1],
          );
      }
    ...
    for (const match of matches) {
      if (
        sentence.words.audience.some((speaker) =>
          fitsIn(speaker.type, match[1]),
        )
      ) {
        ...
        optionalWordMap[match[0]] =
          VerbConjugationService.getConjugatedVersionOfVerb(
            sentence.words.audience,
            match[1],
          );
      }
    }
    ...
    for (const match of matches) {
      if (match[1] === "system") {
        ...
        for (
          let indexToInsert = 1;
          indexToInsert < allSystems.length;
          indexToInsert = indexToInsert + 3
        ) {
          allSystems = [
            ...allSystems.slice(0, indexToInsert),
            {
              name: "and",
              type: "preposition",
            },
    ...
    for (const match of matches) {
      ...
      if (match[1] === "resilience") {
        resilienceWordMap[match[0]] = WhatIfVariantFormatterService(
          sentence,
          part,
        );
      }
