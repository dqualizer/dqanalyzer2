...
export function LoadTestDetail({ loadTestDefinition, rqaId, loadtestSpecifier, parentMenuRef }: LoadtestProps) {
	return (
		<details>
			<summary className="flex justify-between items-center">
				...
				/>
				<EditLoadTestButton loadtestSpecifier={loadtestSpecifier} parentMenuRef={parentMenuRef} />
			</summary>
			<ul>
				<li>{<StimulusDetail stimulus={loadTestDefinition.stimulus} />}</li>
				<li>{<ParametrizationDetail parametrization={loadTestDefinition.parametrization} />}</li>
				<li>{<ResponseMeasureDetail responseMeasure={loadTestDefinition.response_measure} />}</li>
				<li>{<ResultMetricsDetail resultMetrics={loadTestDefinition.result_metrics} />}</li>
			</ul>
		</details>
	);
}
